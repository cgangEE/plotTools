    ...
    for i, curve in enumerate(curves):
        for key in curve.keys():
            ...
            plt.plot(curve['iteration'], curve[key], \
                    label= key + '--' + curve_id[i] , linewidth=0.5)
            ...
            plt.title(key + '--' + curve_id[i])
            plt.grid(b=True, which='major', color='k', linestyle='-')
            plt.grid(b=True, which='minor', color='k', linestyle=':')
            plt.savefig(key + '--' + curve_id[i] ) 
    
if __name__ == '__main__':
    curvesName = ['log_detviplV4d2_D']
    curvesName.append('log_detviplV4d2_DRoiAlign')
    curves = []
    ...
    aliasCurvesName = ['D']
    aliasCurvesName.append('D-RoiAlign')
